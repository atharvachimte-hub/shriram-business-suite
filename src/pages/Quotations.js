...
// लोगो इम्पोर्ट (assuming logo is one level up)
...
const Quotations = () => {
  ...
  const saveBill = () => {
    ...
    if (clientData.name && clientData.company) {
      ...
      const clientExists = clients.find(c => c.name.toLowerCase() === clientData.name.toLowerCase() && c.company.toLowerCase() === clientData.company.toLowerCase());
      if (!clientExists) {
        clients.push({
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="w-full bg-[#f3f4f6] p-4 md:p-8 font-sans">
      
      {/* 3D Header */}
      <div className="max-w-7xl mx-auto mb-8 flex flex-col md:flex-row justify-between items-center bg-white p-6 rounded-[2.5rem] shadow-xl border border-white no-print">
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8 no-print">
        ...
        <motion.div layout className="lg:col-span-9 bg-white rounded-[4rem] p-10 shadow-2xl border border-white">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-10">
            <input type="text" placeholder="कस्टमरचे नाव" value={clientData.name} onChange={(e)=>setClientData({...clientData, name: e.target.value})} className="w-full p-5 rounded-3xl bg-slate-50 border-none font-bold outline-none shadow-inner" />
            <input type="text" placeholder="कंपनीचे नाव" value={clientData.company} onChange={(e)=>setClientData({...clientData, company: e.target.value})} className="w-full p-5 rounded-3xl bg-slate-50 border-none font-bold outline-none shadow-inner" />
          ...
          <div className="bg-slate-50/50 rounded-[3rem] p-8 mb-8 border border-slate-100 overflow-x-auto">
            <table className="w-full text-left">
              <thead>
                <tr className="text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-200">
                  <th className="pb-5 px-4 text-orange-600">तपशील (Description)</th>
                  <th className="pb-5 text-center">Nag</th>
                  <th className="pb-5 text-center">Dar</th>
                  <th className="pb-3 text-right pr-6">Ekun</th>
                </tr>
              </thead>
              <tbody>
                {clientData.items.map((item, idx) => (
                  <tr key={idx}>
                    ...
                    <td className="py-4 text-right font-black text-slate-800 pr-6 text-xl tracking-tighter italic"> ₹{item.qty * item.rate} </td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="flex flex-col md:flex-row justify-between items-center pt-10 border-t border-slate-50 gap-10">
            <div className="flex gap-4">
               ...
               <div className="p-4 bg-green-50 rounded-3xl border border-green-100 shadow-sm"> <label className="block text-[8px] font-black text-green-400 mb-1 uppercase tracking-widest">Advance</label> <input type="number" value={clientData.advance} onChange={(e)=>setClientData({...clientData, advance: e.target.value})} className="w-24 bg-transparent font-black text-green-700 outline-none text-center" /> </div>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mt-16">
            ...
            <button onClick={exportToExcel} className="p-5 rounded-3xl bg-slate-100 text-blue-600 font-bold flex items-center justify-center gap-2"> <FileSpreadsheet size={18} /> Excel </button>
      ...
      <AnimatePresence>
        {showPreview && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/90 backdrop-blur-xl z-[2000] flex items-center justify-center p-4 overflow-y-auto">
            <div className="bg-white w-full max-w-4xl rounded-[4rem] shadow-2xl relative my-auto print-container">
              <button onClick={() => setShowPreview(false)} className="absolute top-8 right-8 p-4 bg-slate-100 rounded-full hover:bg-red-50 hover:text-red-500 no-print"> <X size={24} /> </button>
              
              <div className="p-12 md:p-16" id="print-area">
                <div className="flex justify-between items-center mb-12 border-b-8 border-slate-50 pb-10">
                  <div className="flex items-center gap-6">
                    <img src={logo} alt="Logo" className="w-20 h-20 object-contain bg-black rounded-3xl p-1 shadow-2xl border-2 border-orange-500" />
                    <div> <h2 className="text-3xl font-black text-slate-800 tracking-tighter uppercase italic">{myProfile.company}</h2> <p className="text-[10px] font-bold text-slate-400 tracking-widest uppercase">{myProfile.location}</p> </div>
                  </div>
                  <div className="text-right"> <h3 className="text-5xl font-black text-orange-600/10 uppercase italic tracking-tighter mb-[-5px]">{activeTab}</h3> <p className="text-[10px] font-black text-slate-800 tracking-widest">#{clientData.docId} | {clientData.date}</p> </div>
                </div>

                <div className="mb-10">
                    <div className="p-8 bg-slate-50 rounded-[2.5rem] border-l-[12px] border-orange-600 w-1/2">
                        <p className="text-[8px] font-black text-slate-300 uppercase mb-2 tracking-[0.3em]">Client Bill To:</p>
                        <h4 className="font-black text-slate-800 text-2xl tracking-tight uppercase italic">{clientData.name || '---'}</h4>
                        <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">{clientData.company}</p>
                    </div>
                </div>
                ...
                <table className="w-full mb-10">
                  <thead>
                    <tr className="border-b-2 border-slate-100 text-left">
                      <th className="py-4 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Description</th>
                      <th className="py-4 text-[10px] font-black text-slate-400 uppercase text-center">Nag</th>
                      <th className="py-4 text-[10px] font-black text-slate-400 uppercase text-center">Rate</th>
                      <th className="py-4 text-[10px] font-black text-slate-400 uppercase text-right pr-4">Amount</th>
                    </tr>
                  </thead>
                  <tbody>
                    {clientData.items.map((item, i) => (
                      <tr key={i} className="border-b border-slate-50">
                        <td className="py-6 font-black text-slate-700 text-xl italic tracking-tight uppercase">{item.desc || 'Services'}</td>
                        <td className="py-6 font-black text-slate-600 text-center text-lg">{item.qty}</td>
                        <td className="py-6 font-black text-slate-600 text-center text-lg">₹{item.rate}</td>
                        <td className="py-6 font-black text-slate-900 text-right pr-4 text-2xl tracking-tighter italic">₹{item.qty * item.rate}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                ...
                <div className="flex justify-end">
                  <div className="w-80 p-8 bg-slate-50 rounded-[3rem] space-y-3 shadow-inner">
                    <div className="flex justify-between text-[10px] font-bold text-slate-400 uppercase tracking-widest"> <span>Subtotal</span> <span>₹{subtotal}</span> </div>
                    <div className="flex justify-between text-[10px] font-bold text-slate-400 uppercase tracking-widest"> <span>Tax ({clientData.gst}%)</span> <span>₹{tax}</span> </div>
                    <div className="flex justify-between pt-4 border-t-2 border-slate-200 text-3xl font-black text-slate-900 tracking-tighter italic"> <span>Total</span> <span className="text-orange-600">₹{finalTotal}</span> </div>
                  </div>
                </div>
              </div>

              <div className="bg-slate-50 p-8 flex justify-center no-print">
                <button onClick={() => window.print()} className="bg-black text-white px-12 py-5 rounded-[2rem] font-black text-[10px] uppercase tracking-widest flex items-center gap-4 shadow-2xl hover:scale-105 transition-all"> <Printer size={18} /> Print / Save PDF </button>
              </div>
            </div>
          </motion.div>
        )}
